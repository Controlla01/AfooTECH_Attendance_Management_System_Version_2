...
Public Class markAttendance
    ...
    Public AttendanceMode As String

    Private Sub markAttendance_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        SafeUpdateLabel(markAttendanceStatusLabel, AttendanceMode & " Mode - Scanner ready. Awaiting finger...", Color.DarkOrange)
    ...
    Private Sub SafeUpdateLabel(ByVal label As Control, ByVal text As String, ByVal color As Color)
        If label Is Nothing OrElse label.IsDisposed OrElse label.Disposing Then Exit Sub
        ...
        If label.InvokeRequired Then
            Try
                label.Invoke(Sub()
                             ...
                             End Sub)
            Catch ex As Exception
                MessageBox.Show("Error in SafeUpdateLabel: " & ex.Message)
            End Try
        Else
            Try
                If Not label.IsDisposed Then
   ...
   Public Sub OnComplete(ByVal Capture As Object, ByVal ReaderSerialNumber As String, ByVal Sample As DPFP.Sample) Implements DPFP.Capture.EventHandler.OnComplete
        ...
        Try
            ...
            While reader.Read()
                ...
                If result.Verified Then
                    ...
                    If AttendanceMode = "Check-Out" AndAlso timeSlot = "Check-In" Then
                        SafeUpdateLabel(markAttendanceStatusLabel, "Check-In first before Checking-Out.", Color.Red)
                        AutoCloseFormAfterDelay()
                        Exit Sub
                    End If

                    If AttendanceMode = "Check-In" AndAlso timeSlot = "Already Checked Out" Then
                        SafeUpdateLabel(markAttendanceStatusLabel, "You have already checked out today.", Color.Red)
                        AutoCloseFormAfterDelay()
                        Exit Sub
                    End If

                    If AttendanceMode <> timeSlot Then
                        SafeUpdateLabel(markAttendanceStatusLabel, "Already marked for Attendance.", Color.Red)
                        AutoCloseFormAfterDelay()
                        Exit Sub
                    End If
                    ...
                    If markAttendanceTimeSlot.InvokeRequired Then
                        markAttendanceTimeSlot.Invoke(Sub()
                                                          markAttendanceTimeSlot.ForeColor = If(timeSlot = "Check-In", Color.Green, Color.Maroon)
                                                      End Sub)
                    Else
                        markAttendanceTimeSlot.ForeColor = If(timeSlot = "Check-In", Color.Green, Color.Maroon)
                    End If
    ...
    Private Sub AutoCloseFormAfterDelay()
        ...
        AddHandler closeTimer.Tick, Sub()
                                        ...
                                        Try
                                            ...
                                            End If
                                        Catch ex As Exception
                                            MessageBox.Show("Error stopping scanner before closing: " & ex.Message)
                                        End Try
